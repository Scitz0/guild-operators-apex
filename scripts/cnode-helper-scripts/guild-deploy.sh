...
usage() {
  cat <<-EOF >&2
		...
		Usage: $(basename "$0") [-n <afpm|afpt>] [-p path] [-t <name>] [-b <branch>] [-u] [-s [p][b][l][m][d][c][o][w][x][f][s]]
		...
		-n    Connect to specified network instead of afpm (prime mainnet) network (Default: connect to cardano afpm network) eg for prime testnet: -n afpt
...
download_cnodebins() {
  ...
  if [[ "${SKIP_DBSYNC_DOWNLOAD}" == "N" ]]; then
    echo -e "\n  Downloading Cardano DB Sync 13.6.0.5 archive from share.koios.rest.."
    curl -m 200 -sfL "https://share.koios.rest/api/public/dl/xFdZDfM4/bin/cardano-db-sync-13.6.0.5-$(uname -m).tar.gz" -o cnodedbsync.tar.gz || err_exit "  Could not download cardano-db-sync from release artefacts on GitHub!"
...
populate_cnode() {
  ...
  if [[ ${NETWORK} =~ ^(afpm|afpt)$ ]]; then
    ...
    err_exit "Unknown network specified! Kindly re-check the network name, valid options are: afpm & afpt."
  fi
